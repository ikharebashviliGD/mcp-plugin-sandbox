#!/usr/bin/env bash
# SessionStart hook for sandbox-web plugin.
#
...
#   2. Apply any pending plugin update for THIS plugin only (sandbox-web).
#      The sibling sandbox-mobile plugin has its own hook that updates itself.
#
...
LOG_FILE="/tmp/sandbox-web-hook.log"
MARKETPLACE_NAME="mcp-plugin-sandbox"
PLUGIN_QUALIFIED="sandbox-web@mcp-plugin-sandbox"

{
	echo "---"
	echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] SessionStart hook fired (sandbox-web)"
	echo "[hook:session-start fired plugin=sandbox-web]"
